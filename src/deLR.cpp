...
//' @description
//' This function performs decentralized Lasso regression using proximal gradient descent with consensus penalties.
...
//' @return A list containing the estimated coefficients, error history, and results for each iteration.
//' @export
// [[Rcpp::export]]
Rcpp::List deLR(arma::mat &X, arma::vec &y,
...
s_vec = arma::eig_sym(arma::trans(X.rows(idx)) * X.rows(idx) / n)*tau_penalty_factor;
   ...
   for (int ilambda = 0; ilambda < lambda_array.n_elem; ++ilambda) {
     Rcpp::checkUserInterrupt(); // Check for user interruption
     ...
     for (int t = 0; t < T_inner; ++t) {
       ...
       for (int j = 0; j < m; ++j) {
        ...
        arma::vec update_vec = omega(j) * (rho(j) * B_inner_old.col(j)
                                           ...
                                           - P_beta.col(j)
                                           + tau_penalty_beta * arma::sum(arma::repmat(B_inner_old.col(j), 1, neighbors_j.n_elem) + B_inner_old.cols(neighbors_j), 1));
   ...
   return Rcpp::List::create(
     ...
     Rcpp::Named("history") = Rcpp::List::create(Rcpp::Named("errors_inner") = errors_inner),
